    ...
    for (let i = 0; i < cellText.length; i++) {
      ...
      here: for (let j = 0; j < data[currentSheetIndex - 1].length; j++) {
        for (let k = 0; k < data[currentSheetIndex - 1][j].length; k++) {
          // console.log(activeCell.id, data[currentSheetIndex - 1][j][k].id);
          // if (activeCell.id === data[currentSheetIndex - 1][j][k].id) {
          //   currentCell = data[currentSheetIndex - 1][j][k];
          // }
          // console.log(data[currentSheetIndex - 1][j].id);
          if (
